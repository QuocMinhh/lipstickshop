        ...
        <div class="container">
            <header>
                ...
                <div class="icons">
                    ...
                    $admin = mysqli_query($connectdb, 'select * from taikhoan where name_tk ="admin"');
                    $in4_admin = mysqli_fetch_array($admin);
                    $email_signin = $_SESSION["email-signin"];
                    $password_signin = $_SESSION["password-signin"];

                    while ($login = mysqli_fetch_array($information)) {
                        if ($email_signin == $login["email_tk"] && $password_signin == $login["password_tk"]) {
                            if ($login["name_tk"] == $in4_admin["name_tk"]) {
                                echo "<a href='../lipstick/admin/admin-products.php'
                  ><i class='bx bxs-wrench'></i></a>";
                            }
                        }
                    }
                    ?>
                        <a href="cart.php"><i class="fas fa-shopping-cart"> </i></a>
                        <a href="bill.php"><i class='bx bxs-shopping-bag'></i></a>
                        ...
                        <div class="selectWrapper">
                            ...
                            $admin = mysqli_query($connectdb, 'select * from taikhoan where name_tk ="admin"');
                            $in4_admin = mysqli_fetch_array($admin);
                            $email_select = $_SESSION["email-signin"];
                            $password_select = $_SESSION["password-signin"];

                            while ($login = mysqli_fetch_array($information)) {

                                if ($email_select == $login["email_tk"] && $password_select == $login["password_tk"]) {

                                    if ($login["name_tk"] == $in4_admin["name_tk"]) {

                                        echo "<div class='bottomBorder'>
                                         <a href='../lipstick/account.php'>" . $in4_admin['name_tk'] . "</a>
                                     </div>
                                     <div><a href='../lipstick/account.php'>Logout</a></div>
                                     <div><a href=../lipstick/account.php'>Forget</a></div>
                                     <div><a href=''>Supports</a></div>";
                                    } else {
                                        echo "<div class='bottomBorder'>
                                         <a href='../lipstick/account.php'>" . $login['name_tk'] . "</a>
                                     </div>
                                     <div><a href='../lipstick/account.php'>Logout</a></div>
                                     <div><a href='../lipstick/account.php'>Forget</a></div>
                                     <div><a href='#'>Supports</a></div>
